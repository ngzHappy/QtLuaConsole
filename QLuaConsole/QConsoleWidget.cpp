...
QConsoleWidget::_pf<void, UpdatePromptWidgetPositon>(
    ...
    int sbv = sb->value();
    sbv = (std::max<int>)(-9999,thisp->thisp->positionY-sbv );
...
QConsoleWidget::QConsoleWidget(QWidget * p) :TP(p) {
	...
	/*设置默认字体*/
    {
        ...
        thisp->textCharFormat =
                _cursor.charFormat() ;
		QFont font_ = thisp->textCharFormat.font();
		font_.setPixelSize(20);
		thisp->textCharFormat.setFont(font_);
...
bool QConsoleWidget::eventFilter(QObject * obj_, QEvent * event_) {
	{
		auto * bar_ = qobject_cast<QScrollBar *>(obj_);
		if (bar_ == this->verticalScrollBar()) {
			switch (event_->type())
			{
			case QEvent::Hide: {
				isVBarShow = IsVBarShow::no;
				thisp->visibleBlockRects.clear();
				this->update();
			}; return bar_->event(event_);
			case QEvent::Show: {
				isVBarShow = IsVBarShow::yes;
				thisp->visibleBlockRects.clear();
				this->update();
			}; return bar_->event(event_);
			default:return bar_->event(event_);
			}
		}
	}
	return TP::eventFilter(obj_,event_);
}
...
namespace {
	class IsInnerFunctionEvals;
}

void QConsoleWidget::innerEvals( QList<QConsoleWidgetCommand > evals ) {
	QString commands__ = evals.first().command ;
	if (commands__==":clear") {
		if (evals.size()>1) {
			saveAndClear( evals.at(1).command ) ;
		}
		else {
			saveAndClear();
		}
...
template<> bool
QConsoleWidget::_pf<bool, IsInnerFunctionEvals>(
	QConsoleWidget * thisp
	) {
	const static std::set<QString> innerCommands({ QString(":clear") });
	if (thisp->commands.empty()) { return false; }
	if (thisp->commands.size()==1) {
		auto command__ = (*(thisp->commands.begin())).command;{
			auto c__ = command__.split(QRegExp("\\s+"), QString::SkipEmptyParts);
			if (c__.empty()) { return false; }
			command__ = *c__.begin();
			if (innerCommands.count(command__)>0) {
				thisp->commands.clear();
				for (const auto & i:c__) {
					QConsoleWidgetCommand _temp_;
					_temp_.command = i;
					thisp->commands.push_back(std::move(_temp_));
				}
				thisp->innerEvals(thisp->commands);
				return true;
			}
		}
	}
    ...
    {//TODO: evals
        if (key_==Qt::Key_Return) {
            if (e->modifiers()==0) {
				...
				如果是内部命令就不调用外部命令
				*/
				if (QConsoleWidget::_pf<bool, IsInnerFunctionEvals>(thisp)==false ) {
...
void QConsoleWidget::beginEvals() {
    isEvaling_ = true;
    {
...
paintEvent(QPaintEvent *) {
    ...
    painter.fillRect(this->rect(), QColor(255, 122, 32));
}
...
QConsoleWidget::_pf<void, UpdateOnEditorBlockFormat>(
    ...
    if(block_.isValid()) {
		...
		{
			QTextCursor cursor_ = thisp->textCursor();
...
void QConsoleWidget::saveAndClear() {

{
	...
	if (isLogFileSet) {
		auto block_ = document_->firstBlock();
		do {
			QFile file__(this->logFile);
			if (file__.open(  
				 QIODevice::Append
				|QIODevice::Text
				) == false) {
			...
			QTextStream stream__(&file__);
			stream__.setCodec( QTextCodec::codecForName("UTF-8") );
			stream__<<"\r\n"<<QDateTime::currentDateTime().\
				toString("yyyy-MM-dd hh:mm:ss ddd")<<"\n";
			while (block_.isValid()) {
				QString block_text = block_.text();
				stream__ << block_text <<"\n";
				block_ = block_.next();
			}
		} while (0);
	...
	*/
	...
	/*reinit*/
	/*设置默认提示符*/
	this->promptBeginPos_ = 0;
	this->prompt_ = this->prompt_.simplified();
	this->promptEndPos_ = this->prompt_.size();
